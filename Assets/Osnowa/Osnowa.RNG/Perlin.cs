namespace Osnowa.Osnowa.RNG
{
	...
	public class Perlin
	{
		public static float Noise(float x, float y, float z, int octaves, float persistence)
		{
			float total = 0;
			float frequency = 1;
			float amplitude = 1;
		...
		public static float Noise(Position position)
		{
			return Noise(position.x, position.y);
		}

		public static float MaxValueFromNoise(int octaves, float persistence)
		{
			float maxValueFromNoise = 0f;
			for (int octaveIndex = 1; octaveIndex <= octaves; octaveIndex++)
			{
				maxValueFromNoise += (float)Math.Pow(persistence, octaveIndex - 1);
			}
			return maxValueFromNoise;
		}

		public static float Noise(Position position, int octaves, int persistence)
		{
			return Noise(position.x, position.y, octaves, persistence);
		}

		public static float Noise(float x, float y)
		{
			const float xOffset = 65536.23132f;
			const float yOffset = 65536.43534f;

			return Noise(x + xOffset, y + yOffset, 65536.324235f);
		}

		public static float Noise(float x, float y, int octaves, float persistence)
		{
			const float xOffset = 65536;
			const float yOffset = 65536;

			return Noise(x + xOffset, y + yOffset, 65536.324235f, octaves, persistence);
		}
