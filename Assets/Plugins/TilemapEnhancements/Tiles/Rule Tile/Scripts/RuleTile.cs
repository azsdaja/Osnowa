...
namespace Assets.Plugins.TilemapEnhancements.Tiles.Rule_Tile.Scripts
{
	[Serializable]
	[CreateAssetMenu]
	public class RuleTile : TileWithConsistencyClass
	{
			...
			for (int y = -1; y <= 1; y++)
			{
				for (int x = -1; x <= 1; x++)
				{
					if (x != 0 || y != 0)
					{
						...
						if (rule.m_Neighbors[index] == TilingRule.Neighbor.NotThis && (tile == this || !IsDifferentThan(tile)))
						{
							return false;
						}
						if (rule.m_Neighbors[index] == TilingRule.Neighbor.This && tile != this && IsDifferentThan(tile))
						{
							return false;
						}
					}
			...
			for (int y = -1; y <= 1; y++)
			{
				for (int x = -1; x <= 1; x++)
				{
					if (x != 0 || y != 0)
					{
						...
						if (rule.m_Neighbors[index] == TilingRule.Neighbor.This && IsDifferentThan(tile))
						{
							return false;
						}
					}
		...
		private bool IsDifferentThan(TileBase tile)
		{
			var otherWithConsistencyClass = tile as TileWithConsistencyClass;

			if (otherWithConsistencyClass != null)
			{
				if (tile.name != name)
				{
					
				}
				return !IsConsistentWith(otherWithConsistencyClass);
			}
			else return tile != this;
		}
